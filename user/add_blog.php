...
if(isset($_POST['submit_question']))
{
    ...
    $targetFilePath = $targetDir . $fileName; 
         ...
         if(in_array($fileType, $allowTypes)){ 
             ...
             if(move_uploaded_file($_FILES["blog_title_image"]["tmp_name"], "../uploaded_images/blog_title_images/".$date1)){ 
                   $stmt = $conn->prepare("INSERT INTO blog(blog_title,blog_title_image,blog_text,category_id,blog_status,user_id,district_id,added_on)VALUES(:blog_title,:blog_title_image,:blog_text,:category_id,:blog_status,:user_id,:district_id,:added_on)");
                   $executed=$stmt->execute(array(':blog_title' => $blog_title,':blog_title_image' => $date1,':blog_text' => $blog_text,':category_id' => $category_id,':blog_status' => 1,':user_id' =>$row_user_details[0]['user_id'],':district_id' =>$row_user_details[0]['district_id'],':added_on' => date('Y-m-d H:i:s')));
                   if($executed){
  ...
  {
        $stmt = $conn->prepare("INSERT INTO blog(blog_title,blog_text,category_id,blog_status,user_id,district_id,added_on)VALUES(:blog_title,:blog_text,:category_id,:blog_status,:user_id,:district_id,:added_on)");
        $executed=$stmt->execute(array(':blog_title' => $blog_title,':blog_text' => $blog_text,':category_id' => $category_id,':blog_status' => 1,':user_id' =>$row_user_details[0]['user_id'],':district_id' =>$row_user_details[0]['district_id'],':added_on' => date('Y-m-d H:i:s')));
        if($executed)
        {
           echo '<script>alert("Blog added");window.location.href = "all_blogs";</script>';
        }
                                ...
                                <div class="card-body col-md-12 mx-auto border mb-3">
                                  ...
                                  <div class="form-group">
                                    ...
                                    <select name="category_id" class="form-control">
                                        ...
                                        <?php
                                        $stmt_category_list = $conn->prepare("SELECT * FROM category WHERE category_status=1 ORDER BY category_id DESC");
                                  ...
                                  <div class="form-group">
                                    ...
                                    <div class="input-group">
                                      <div class="custom-file">
                                        <input type="file" class="custom-file-input" id="blog_title_image"  name="blog_title_image" accept="image/jpg, image/jpeg, image/png" onchange="loadFile(event)">
                      ...
                      function checkImageDetails(img,file_id)
                      {
                          ...
                          var filePath = fileName.value;
                            ...
                            else
                            {
                                ...
                                if(fileSize<=2000000<?php //echo $row_restrictions[0]['max_file_size_in_kb'];?>)
                                {
